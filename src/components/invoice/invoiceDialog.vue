          ...
          <div class="text-c_sliver">
            <div class="form-area">
              ...
              <p class="text-c_sliver mt-5">依統一發票使用辦法規定：電子發票一經開立，不得任意更改或改開公司發票。(<a class="text-c_dodger_blue"
                  href="https://www.einvoice.nat.gov.tw/" target="_blank">財政部電子發票流程說明</a>)</p>
            </div>
          </div>
        </div>
        <div
          ...
          <button
            :class="`w-[49%] p-2 flex justify-center items-center text-base rounded-[10px] border border-solid ${resetBtnBorderColor} ${resetBtnTextColor}`"
            @click="resetInvoice(currenType)">重置</button>
          <button
