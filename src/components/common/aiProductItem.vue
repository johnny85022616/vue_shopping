      ...
      <ul v-if="itemList" class="list-none"> 
        <li v-for="(item, index) in itemList" :key="index" class="flex justify-between mb-6 border-b border-solid border-c_alto pb-4"
          ...
          <div class="item-img relative w-1/4 h-0 pb-1/4">
            ...
            <span v-if="!item.qtyava" class="squaremask absolute flex justify-center items-center opacity-95 right-0 left-0 bottom-0 w-full h=5 text-white text-center bg-c_sliver">
              ...
              <div class="items-txt">
                ...
                <span v-if="item.price !== null" class="price-box flex justify-between items-center text-sm">
                    <span class="price-box inline-block relative w-2/3 ">
                      ...
                      <p v-if="item.priceSuffix" class="priceSuffix absolute bottom-[-15px] left-2 text-c_special text-2">{{ item.priceSuffix }}</p>
                    </span>
