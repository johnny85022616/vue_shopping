        ...
        <section class="qaRecord relative h-[calc(100vh-46px-10px)]">
          ...
          <section v-if="!isMessageOpen" class="input__box w-full bg-c_mercuryApprox py-5 px-4">
            <div class="categoryBlock mb-2">
              <select id="category" name="category" class="selbox p-1 rounded-[5px] w-[63%] mb-1 text-center" v-model="qaCategory">
            ...
            <div class="textAreaBlock flex items-end justify-between">
              <textarea v-model="clientInput" ref="autoGrowTextarea" class="min-h-[100px] overflow-scroll w-4/5 py-1 px-2" rows="1" @keypress="handleKeydown"
            ...
            <template v-if="conversationInfo && conversationInfo.length > 0">
              <div v-for="(v, idx) in conversationInfo" :key="idx" class="content__box py-2">
                <div v-if="v.replyId && v.reply"
                  ...
                  <img class="w-[10%] mr-4" src="../../images/icons/xiuwu.png" />
                  <p
                ...
                <div v-else-if="!v.replyId && v.question"
                  ...
                  <p class="client-text bg-c_light_blue shadow-[-1px_2px_3px_rgba(173,173,173,1)] relative break-all max-w-[70%] py-2 px-4 rounded-[8px]">{{ v.question
